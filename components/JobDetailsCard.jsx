...
const JobDetailsCard = () => {
    ...
    return (
        <div className='flex flex-col md:flex-row md:gap-10'>
            <div className={`px-4 py-5 mt-8 rounded-2xl md:w-[70%] ${isDark ? 'bg-[##1C1C24]' : 'bg-white'}`}>
                <div className='w-full relative'>
                    <Image src={background} alt='background' className='w-full rounded-t-3xl mt-2 md:max-h-48' />
                    <div className='absolute bg-[#FAFAFB] bottom-[-30px] rounded-lg border-1 left-4'>
                        <img src={currentJobData?.employer_logo} alt={currentJobData?.employer_name} className='object-contain p-[1px] w-[60px] rounded-lg' />
                    </div>
                </div>
                <div className='mt-14 px-4 flex justify-between items-center'>
                    <div className='flex gap-4 items-center'>
                        <h2 className={`font-semibold text-lg ${isDark ? 'text-white' : 'text-[#171725]'}`}>{currentJobData?.job_title}</h2>
                    ...
                    <div className='flex items-center gap-4'>
                        ...
                        <button className={`bg-transparent text-[#696974] border px-8 py-2 rounded-xl text-lg hidden md:flex ${isDark && 'border-[#696974]/20'}`}>Message</button>
                ...
                <div className={`w-[70%] p-3 flex flex-col md:flex-row md:justify-start md:gap-32 rounded-2xl mt-7 mx-4 ${isDark ? 'bg-[#21212B]' : 'bg-[#FAFAFB]'}`}>
                    <div className='flex justify-between md:gap-32'>
                        <div className='flex flex-col gap-2'>
                            <p className='text-[#92929D]'>Experience</p>
                            <p className={`font-semibold ${isDark && 'text-white'}`}>Minimum 1 Year</p>
                        </div>
                        <div className='flex flex-col gap-2'>
                            <p className='text-[#92929D]'>Work Level</p>
                            <p className={`font-semibold ${isDark && 'text-white'}`}>Senior Level</p>
                        </div>
                    </div>
                    <div className={`border-b my-4 md:hidden ${isDark && 'border-[#1C1C24]'}`} />
                    <div className='flex justify-between md:gap-32'>
                        <div className='flex flex-col gap-2'>
                            <p className='text-[#92929D]'>Employee Type</p>
                            <p className={`font-semibold ${isDark && 'text-white'}`}>{currentJobData?.job_employment_type}</p>
                        </div>
                        <div className='flex flex-col gap-2'>
                            <p className='text-[#92929D]'>Offer Salary</p>
                            <p className={`font-semibold ${isDark && 'text-white'}`}>$2150,0 / M</p>
                        </div>
                    </div>
                </div>
                <div className='px-4 mt-4 flex gap-10 md:hidden'>
                    <button className='bg-[#0BAB7C] text-white px-8 py-2 rounded-xl text-lg'>Apply Now</button>
                    <button className={`bg-transparent text-[#696974] border px-8 py-2 rounded-xl text-lg ${isDark && 'border-[#696974]/20'}`}>Message</button>
                </div>
                <div className='mt-8 px-4 flex flex-col gap-2 mb-14'>
                    <h2 className={`font-semibold text-lg ${isDark ? 'text-white' : 'text-[#171725]'}`}>About The Job</h2>
                    <p className='text-[#696974] font-normal'>{currentJobData?.job_highlights?.Responsibilities[0] ?? 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quod.'}</p>
                </div>
                ...
                <div className='px-4 pt-8'>
                    <h2 className={`font-semibold text-lg ${isDark ? 'text-white' : 'text-[#171725]'}`}>About The Company</h2>
                    <div className='mt-5 flex gap-5 items-center'>
                        <Image src={companylogo} className='w-10 h-10' alt='company' />
                        <div className='flex flex-col gap-1'>
                            <h2 className={`font-semibold text-lg ${isDark && 'text-white'}`}>UI HUT</h2>
