        ...
        <>
            <div className={`px-6 md:px-20 py-6 border-b ${isDark ? 'border-[#21212B]' : 'border-[#E2E2EA]'} text-[#92929D] ${isDark ? 'bg-[#13131A]' : 'bg-white'} fixed top-0 w-full z-50`}>
                <div className='flex justify-between items-center'>
                    <RxHamburgerMenu className='text-2xl flex md:hidden cursor-pointer shrink-0' onClick={() => dispatch(toggleMobile())} />
                    ...
                    <div className='gap-8 font hidden md:flex md:flex-1 md:mx-auto justify-center'>
                        <Link href='/' onClick={() => dispatch(update('home'))}>
                            <div className={`relative ${activePage === 'home' ? 'text-[#0BAB7C] font-bold text-base' : ''}`}>
                                Overview
                                {activePage === 'home' && <div className='h-[2px] w-full bg-[#0BAB7C] bottom-[-32px] left-0 absolute' />}
                            </div>
                        </Link>
                        <Link href='/salaries' onClick={() => dispatch(update('salaries'))}>
                            <div className={`relative ${activePage === 'salaries' ? 'text-[#0BAB7C] font-bold text-base' : ''}`}>
                                Estimated Salaries
                                {activePage === 'salaries' && <div className='h-[2px] w-full bg-[#0BAB7C] bottom-[-32px] left-0 absolute' />}
                            </div>
                        </Link>
                        <Link href='/search' onClick={() => dispatch(update('search'))}>
                            <div className={`relative ${activePage === 'search' ? 'text-[#0BAB7C] font-bold text-base' : ''}`}>
                                Job Search
                                {activePage === 'search' && <div className='h-[2px] w-full bg-[#0BAB7C] bottom-[-32px] left-0 absolute' />}
                            </div>
                        </Link>
                    </div>
